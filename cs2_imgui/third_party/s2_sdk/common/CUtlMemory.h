...
CUtlMemory<T, I>::CUtlMemory(int nGrowSize, int nInitAllocationCount): m_pMemory(0), m_nAllocationCount(nInitAllocationCount), m_nGrowSize(nGrowSize) {
    ...
    if (m_nAllocationCount) {
        m_pMemory = (T*)g_memalloc->Alloc(m_nAllocationCount * sizeof(T));
    }
}
...
void CUtlMemory<T, I>::ConvertToGrowableMemory(int nGrowSize) {
    ...
    if (m_nAllocationCount) {
        ...
        T* pMemory = (T*)g_memalloc->Alloc(nNumBytes);
...
void CUtlMemory<T, I>::Grow(int num) {
    ...
    if (m_pMemory) {
        auto ptr = g_memalloc->Alloc(m_nAllocationCount * sizeof(T));
...
inline void CUtlMemory<T, I>::EnsureCapacity(int num) {
    ...
    if (m_pMemory) {
        m_pMemory = (T*)g_memalloc->ReAlloc(m_pMemory, m_nAllocationCount * sizeof(T));
    } else {
        m_pMemory = (T*)g_memalloc->Alloc(m_nAllocationCount * sizeof(T));
    }
}
...
void CUtlMemory<T, I>::Purge() {
    if (!IsExternallyAllocated()) {
        if (m_pMemory) {
            g_memalloc->Free((void*)m_pMemory);
            m_pMemory = 0;
        }
        m_nAllocationCount = 0;
    }
